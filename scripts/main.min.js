...
(function(angular) {
    ...
    //TODO JQ: deploy UI to QA and smoke test it
    //TODO JQ: deploy UI to prod

    //TODO JQ: fix next meeting when there are not meetings
    ...
    //TODO JQ: add student ministry to age search criteria
            ...
            return {
                ...
                link: function (scope, element, attribute, ngModel) {
                    ngModel.$validators.timeAfter = function (modelValue) {
                        ...
                        return moment(modelValue).isAfter(moment(scope.otherModelValue));
                    };
        ...
        .filter('mailToAll', function() {
            return function(users, subject) {
                if (users && users.length > 0) {
                    var usersWithEmail = users.filter(function(u) {
                        return u.email != null;
                    });
                    var link = "mailto:" + usersWithEmail[0].email + "?subject=" + (subject || "");
                    if (usersWithEmail.length > 1) {
                        link += "&to=";
                        for (var i = 1; i < usersWithEmail.length; i++) {
                            link += (i == 1 ? "" : ",") + usersWithEmail[i].email;
                        }
                    }
                    return link;
                }
...
(function(angular) {
    ...
    angular.module('groups', ['ui.router', 'users', 'ngConstants'])
        .config(["$stateProvider", "partialPath", function ($stateProvider, partialPath) {
            ...
            $stateProvider
                ...
                .state("groups.edit", {
                    ...
                    controller: ["$rootScope", "graceGroupsService", "group", "leaders", EditGroupController],
                    controllerAs: "$ctrl"
                })
                .state("groups.new", {
                    url: "/new",
                    ...
                    controller: ["$rootScope", "graceGroupsService", "leaders", "authUser", NewGroupController],
                    controllerAs: "$ctrl"
                })
                ...
                .state("groups.members.select", {
                    ...
                    controller: ["$state", "graceGroupsService", "group", "members", "limit", GroupMemberSelectController],
                    controllerAs: "$ctrl"
                })
                .state("approvals", {
                    url: "/approvals",
        ...
        .component('graceGroupListing', {
            ...
            bindings: { limit: "<", showSearch: "<", onEditGroup: "&", onGroupMembers: "&", onSelectGroup: "&"},
        ...
        .component('ggGroupForm', {
            ...
            bindings: { $router: '<', group: "<", leaders: "<", onSave: "&", onCancel: "&"},
    ...
    function EditGroupController($rootScope, service, group, leaders) {
        var self = this;
        self.group = group;
    ...
    function NewGroupController($rootScope, service, leaders, authUser) {
        var self = this;
        self.group = {};
        self.leaders = leaders;
    ...
    function GroupMemberSelectController($state, service, group, members, limit) {
        var self = this;
        ...
        self.addMember = function(user) {
            service.addGroupMember(self.group.id, user.id).then(function() {
                ...
                // $state.go("groups.details", {id: group.id});
            }, function(err) {
...
(function(angular) {
    ...
    angular.module('meetingReports', ['ui.router', "angularPromiseButtons", "ngConstants"])
        .config(["$stateProvider", "partialPath", function ($stateProvider, partialPath) {
            $stateProvider
                ...
                .state("meetingReports.new", {
                    url: "/new?groupId",
                    templateUrl: partialPath + "/meeting-reports/new.html",
                    resolve: {
                        group: ["graceGroupsService", "$stateParams", function(service, $stateParams) {
                            return service.group($stateParams.groupId).then(function(group) {
                                return group;
                            });
                        }],
                        members: ["graceGroupsService", "$stateParams", function(service, $stateParams) {
                            return service.groupMembers($stateParams.groupId).then(function(users) {
                                return users.sort(compareLastNames);
                            });
                        }]
                    },
                    controller: ["group", "members", "$state", "graceGroupsService", NewMeetingReportController],
                    controllerAs: "$ctrl"
                })

                .state("meetingReports.details", {
                    url: "/{id:int}/details",
                    templateUrl: partialPath + "/meeting-reports/details.html",
                    resolve: {
                        meeting: ["graceGroupsService", "$stateParams", function(service, $stateParams) {
                            return service.meeting($stateParams.id).then(function(meeting) {
                                return meeting;
                            });
                        }],
                        members: ["graceGroupsService", "meeting", function(service, meeting) {
                            return service.groupMembers(meeting.groupId).then(function(users) {
                                return users.sort(compareLastNames);
                            });
                        }],
                        group: ["graceGroupsService", "meeting", function(service, meeting) {
                            return service.group(meeting.groupId).then(function(group) {
                                return group;
                            });
                        }]
                    },
                    controller: ["meeting", "group", "members", MeetingDetailController],
                    controllerAs: "$ctrl"
                });
        }]);

    function NewMeetingReportController(group, members, $state, service) {
        var self = this;
        self.group = group;
        self.members = members;
        ...
        self.meeting = {groupId: group.id};

        if (self.group.schedule.startTime) {
            self.time = moment(self.group.schedule.startTime, "HH:mm").toDate();
        }

        self.addPrayerRequest = function() {
            self.prayerRequests.push({id: self.prayerRequests[self.prayerRequests.length-1].id+1});
        };

        self.attendeeCount = function() {
            var regularCount =  self.regularAttendees.filter(function(a) {return a.present;}).length;
            return regularCount + self.guests.length;
        };

        self.addGuest = function() {
            var id = self.guests.length == 0 ? 0 : self.guests[self.guests.length-1].id+1;
            self.guests.push({id: id});
        };

        self.save = function (form) {
            if (form.$valid) {
                setAttendanceOnMeeting();
                setPrayerRequestsOnMeeting();
                setTimeOnMeeting();
                return service.createMeeting(self.meeting).then(function() {
                    $state.go("groups.details", {id: group.id, show: "reports"});
                }, function (errors) {
                    console.log(errors);
                    Util.alert("Failed to save the meeting report. Please try again later.");
                });
            } else {
                var firstError = $(".has-error:first");
                if (firstError.length > 0) {
        ...
        function initPrayerRequests() {
            self.prayerRequests = [{id: 0}, {id: 1}];
        }

        function initAttendance() {
            self.regularAttendees = members.map(function(member) {
                return {member: member, present: true};
            });

            self.guests = [];
        }

        function setAttendanceOnMeeting() {
            self.meeting.attendance = self.regularAttendees
                .filter(function(a) {return a.present;})
                .map(function(a) { return { userId: a.member.id }; });

            var guestAttendees = self.guests.map(function(g) {
                return {
            ...
            Array.prototype.push.apply(self.meeting.attendance, guestAttendees);
        }

        function setPrayerRequestsOnMeeting() {
            self.meeting.prayerRequests = self.prayerRequests
                .filter(function(p) {return !Util.isEmpty(p.txt);})
                .map(function(p) {return p.txt;});
        }

        function setTimeOnMeeting() {
            self.meeting.time = moment(self.date).hour(self.time.getHours()).minute(self.time.getMinutes()).format("YYYY-MM-DDTHH:mm");
        }

    }

    function MeetingDetailController(meeting, group, members) {
        var self = this;
        ...
        function initAttendance() {
            self.regularAttendees = members.map(function(member) {
                return {member: member, present: meeting.attendance.filter(function(a) {return a.userId == member.id;}).length > 0};
            });

            self.guests = meeting.attendance.filter(function(a) {
                if (Util.isEmpty(a.userId)) {
                    return true;
                } else {
...
(function (angular) {
    ...
    function GraceGroupsService($http, $q, $cookies, secureCookies, serviceUrl, Upload) {
        ...
        function supplementMeeting(meeting) {
            meeting.memberCount = function() {
                return this.attendance.filter(function(a) { return !Util.isEmpty(a.userId); }).length;
            };
            meeting.guestCount = function() {
                return this.attendance.filter(function(a) { return Util.isEmpty(a.userId); }).length;
            };
